...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: campaign, error } = await supabase
    .from("campaigns").select("id, name, message, image_url, buttons, poll_question, agent_id, status").eq("id", id).single();
  ...
  return NextResponse.json({
    ...
    pollQuestion: campaign.poll_question || null,
    pollOptions: campaign.buttons || null,
