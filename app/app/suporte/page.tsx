...
import { LifeBuoy, Plus, Send, Loader2, MessageSquare, X } from "lucide-react";
...
export default function SuportePage() {
  ...
  // Polling simples para novas respostas do admin
  useEffect(() => {
    if (!user) return;
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      const { data: ticket, error } = await supabase
        .from("support_tickets")
        .insert({ user_id: user.id, user_email: user.email, subject: newSubject.trim() })
        .select()
        .single();
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      // Reabre o chamado se o usuário responder após o admin ter respondido.
      ...
      if (ticket && ticket.status !== "open") {
        ...
        await supabase.from("support_tickets").update({ updated_at: new Date().toISOString() }).eq("id", activeId);
      }
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-8rem)] flex items-center justify-center text-muted-foreground gap-2">
        <Loader2 className="h-5 w-5 animate-spin" /> Carregando suporte...
      </div>
    );
  }
    ...
    <div className="h-[calc(100vh-8rem)] flex rounded-xl border border-border bg-card overflow-hidden">
      ...
      <div className="w-80 border-r border-border flex flex-col shrink-0 bg-background/50">
        <div className="p-4 border-b border-border flex items-center justify-between gap-2">
          <div>
            <h2 className="font-semibold text-sm flex items-center gap-2"><LifeBuoy className="h-4 w-4 text-brand-500" /> Suporte</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Fale com a nossa equipe</p>
          </div>
          <Button size="sm" className="gap-1.5 shrink-0" onClick={() => setShowNewForm(v => !v)}>
            <Plus className="h-3.5 w-3.5" /> Novo
          </Button>
        </div>

        {showNewForm && (
          <div className="p-4 border-b border-border space-y-2 bg-secondary/10">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium">Abrir chamado</h4>
              <button onClick={() => setShowNewForm(false)} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
            </div>
            <Input placeholder="Assunto" value={newSubject} onChange={e => setNewSubject(e.target.value)} className="text-sm" />
            <Textarea placeholder="Descreva sua dúvida ou problema..." value={newMessage} onChange={e => setNewMessage(e.target.value)} className="text-sm min-h-[80px]" />
            <Button size="sm" className="w-full" onClick={handleCreateTicket} disabled={creating || !newSubject.trim() || !newMessage.trim()}>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-card">
        {activeTicket ? (
            ...
            <div className="h-16 border-b border-border flex items-center justify-between px-6 shrink-0">
              <div>
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  Status: <Badge variant={STATUS_LABEL[activeTicket.status]?.variant || "secondary"} className="text-[10px] px-1.5 py-0 h-4 ml-1">
                ...
                <div key={m.id} className={`flex max-w-[80%] ${m.is_admin ? '' : 'ml-auto justify-end'}`}>
                  <div className="flex flex-col">
                    <div className={`text-[10px] text-muted-foreground mb-1 ${m.is_admin ? 'ml-1' : 'mr-1 text-right'}`}>
                      {m.is_admin ? "Equipe BizPilot" : "Você"} · {formatTime(m.created_at)}
                    </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground gap-2">
            ...
            <span>Abra um novo chamado para falar com a nossa equipe.</span>
          </div>
        )}
      </div>
    </div>
